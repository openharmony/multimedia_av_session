...
void AVSessionManagerImpl::RegisterServiceStateListener(sptr<ISystemAbilityManager> mgr)
{
    ...
    if (serviceListener_ == nullptr && serviceStartCallback_) {
        serviceListener_ = new(std::nothrow) ServiceStatusListener([this](bool isAddSystemAbility) {
            OnServiceStateChange(isAddSystemAbility);
        });
...
int32_t AVSessionManagerImpl::CreateSessionWithExtra(const std::string& tag, int32_t type, const std::string& extraInfo,
                                                     const AppExecFwk::ElementName& elementName,
                                                     std::shared_ptr<AVSession>& session)
{
    AVSESSION_TRACE_SYNC_START("AVSessionManagerImpl::CreateSessionWithExtra with ret");
    if (tag.empty() || elementName.GetBundleName().empty() || elementName.GetAbilityName().empty() ||
        extraInfo.empty()) {
        SLOGE("param is invalid");
        return ERR_INVALID_PARAM;
...
int32_t AVSessionManagerImpl::SendSystemAVKeyEvent(const MMI::KeyEvent& keyEvent, const AAFwk::Want &wantParam)
{
    AVSESSION_TRACE_SYNC_START("AVSessionManagerImpl::SendSystemAVKeyEvent");
    if (!keyEvent.IsValid()) {
        SLOGE("keyEvent is invalid");
    ...
    auto service = GetService();
    return service ? service->SendSystemAVKeyEvent(keyEvent, wantParam) : ERR_SERVICE_NOT_EXIST;
}
...
ServiceStatusListener::ServiceStatusListener(const std::function<void(bool)>& callback)
    : callback_(callback)
{
    SLOGD("construct");
}
