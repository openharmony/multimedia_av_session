...
napi_value NapiAVSessionManager::GetHistoricalSessionDescriptors(napi_env env, napi_callback_info info)
{
    ...
    auto input = [env, context](size_t argc, napi_value* argv) {
        if (argc == ARGC_ONE && !NapiUtils::TypeCheck(env, argv[ARGV_FIRST], napi_undefined)
            ...
            CHECK_ARGS_RETURN_VOID(context, context->status == napi_ok, "invalid arguments",
                NapiAVSessionManager::errcode_[ERR_INVALID_PARAM]);
            CHECK_ARGS_RETURN_VOID(context, (context->maxSize_ == MEDIACONTROL_HISTORICAL_NUM &&
...
napi_value NapiAVSessionManager::OnActiveSessionChanged(napi_env env, napi_callback_info info)
{
    auto context = std::make_shared<ContextBase>();
    if (context == nullptr) {
        SLOGE("OnActiveSessionChanged failed : no memory");
        NapiUtils::ThrowError(env, "OnActiveSessionChanged failed : no memory",
    ...
    auto input = [&callback, env, &context](size_t argc, napi_value *argv) {
        ...
        err = PermissionChecker::GetInstance().CheckPermission(PermissionChecker::CHECK_SYSTEM_PERMISSION);
        CHECK_ARGS_RETURN_VOID(context, err == ERR_NONE, "Check system permission error",
            NapiAVSessionManager::errcode_[ERR_NO_PERMISSION]);
        err = PermissionChecker::GetInstance().CheckPermission(PermissionChecker::CHECK_MEDIA_RESOURCES_PERMISSION);
        CHECK_ARGS_RETURN_VOID(context, err == ERR_NONE, "Check media resources permission error",
            NapiAVSessionManager::errcode_[ERR_PERMISSION_DENIED]);
    ...
    bool isAddSuccess = false;
    {
        ...
        isAddSuccess = listener_ != nullptr &&
            listener_->AddCallback(env, NapiSessionListener::EVENT_ACTIVE_SESSION_CHANGED, callback) == napi_ok;
    }
...
napi_value NapiAVSessionManager::OffActiveSessionChanged(napi_env env, napi_callback_info info)
{
    auto context = std::make_shared<ContextBase>();
    if (context == nullptr) {
        SLOGE("OffActiveSessionChanged failed : no memory");
    ...
    bool isDelSuccess = false;
    {
        ...
        isDelSuccess = listener_ != nullptr &&
            listener_->RemoveCallback(env, NapiSessionListener::EVENT_ACTIVE_SESSION_CHANGED, callback) == napi_ok;
    }
...
{
    std::string err;
    const uint32_t systemId = 226;
    const uint32_t AvSessionModuleId = 18;
    const uint32_t CastEngineModuleId = 19;
    const uint32_t systemIdOffset = 21;
    const uint32_t moduleIdOffset = 16;

    auto makeAvSessionErrorCode = [](int error) {
        return (systemId << systemIdOffset) | (AvSessionModuleId << moduleIdOffset) |
               static_cast<uint32_t>(std::abs(error));
    };
    auto makeCastEngineErrorCode = [](int error) {
        return (systemId << systemIdOffset) | (CastEngineModuleId << moduleIdOffset) |
               static_cast<uint32_t>(std::abs(error));
    };
    if (error == ERR_NO_PERMISSION) {
        ...
        auto errorCode = makeAvSessionErrorCode(error);
        ...
        auto errorCode = makeCastEngineErrorCode(error);
...
napi_status NapiAVSessionManager::OnDeviceStateChanged(napi_env env, napi_value callback)
{
    SLOGI("OnDeviceStateChanged AddCallback");
    CHECK_AND_RETURN_RET_LOG(listener_ != nullptr, napi_generic_failure, "callback has not been registered");
...
napi_status NapiAVSessionManager::OffDeviceStateChanged(napi_env env, napi_value callback)
{
