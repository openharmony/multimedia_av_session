    ...
    auto executor = [context]() {
        ...
        if (ret != AVSESSION_SUCCESS) {
            if (ret == ERR_SESSION_NOT_EXIST) {
                ...
                context->errMessage = "SetDisplaySurface failed : native command send nums overload,Controls the frequency of sending self-query and control commands";
            } else if (ret == ERR_NO_PERMISSION) {
...
std::string NapiAVCastController::GetSendControlCommandErrMsg(int32_t error)
{
    ...
    switch (error) {
        ...
        case ERR_COMMAND_SEND_EXCEED_MAX:
            msg = "SendControlCommand failed : native command send nums overload,Controls the frequency of sending self-query and control commands";
            break;
        default:
