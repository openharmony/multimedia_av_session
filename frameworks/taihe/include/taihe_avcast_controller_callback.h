...
#ifndef TAIHE_AVCAST_CONTROLLER_CALLBACK_H
#define TAIHE_AVCAST_CONTROLLER_CALLBACK_H
...
using MSErrCode = OHOS::ErrCode;
class TaiheAVCastControllerCallback : public OHOS::AVSession::AVCastControllerCallback,
    public std::enable_shared_from_this<TaiheAVCastControllerCallback> {
public:
    enum {
    ...
    static constexpr MSErrCode MS_ERR_OFFSET = OHOS::ErrCodeOffset(30, MS_MODULE); // 30 is SUBSYS_MULTIMEDIA
    // media api9 error code
    enum MediaServiceExtErrCodeAPI9 : MSErrCode {
    ...
    enum CastExtErrCodeAPI13 : MSErrCode {
        // Generic error
        ...
        CAST_IOERR_EXT_API13_NO_CONTENTS = 6612100,
    ...
    enum MediaServiceErrCode : MSErrCode {
        MSERR_OK                = OHOS::ERR_OK,
    ...
    enum CastErrCode : MSErrCode {
        ...
        ERROR_CODE_IO_NO_CONTENTS = 2100,
    ...
    TaiheAVCastControllerCallback();
    ~TaiheAVCastControllerCallback() override;

    void OnCastPlaybackStateChange(const OHOS::AVSession::AVPlaybackState &state) override;
    void OnMediaItemChange(const OHOS::AVSession::AVQueueItem &avQueueItem) override;
    ...
    int32_t onDataSrcRead(std::shared_ptr<OHOS::AVSession::AVSharedMemory> mem, uint32_t length, int64_t pos) override;

    bool IsCallbacksEmpty(int32_t event);

    int32_t ReadDataSrc(std::shared_ptr<OHOS::AVSession::AVSharedMemory> mem, uint32_t length, int64_t pos);
...
private:
    ...
    std::function<bool()> CheckCallbackValid(int32_t event, const std::list<std::shared_ptr<uintptr_t>>::iterator &ref);

    void HandleErrorEvent(int32_t event, const int32_t errorCode, const std::string &errorMsg);

    void ExecuteErrorCallback(std::shared_ptr<uintptr_t> method, int32_t errorCode, const std::string errorMsg) const;
    ...
    struct DataContext {
        array<uint8_t> keyRequestData;
        string assetId;
        array<string>cmds;
    };

    std::mutex lock_;
    std::list<std::shared_ptr<uintptr_t>> callbacks_[EVENT_CAST_TYPE_MAX] {};
...
#endif // TAIHE_AVCAST_CONTROLLER_CALLBACK_H