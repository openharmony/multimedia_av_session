...
static bool GetSampleRate(ani_env *env, ani_object aniObj, std::set<OHOS::AudioStandard::AudioSamplingRate> &out)
{
    ...
    if (valArray.size() > 0) {
        out = {static_cast<OHOS::AudioStandard::AudioSamplingRate>(valArray[0])};
    }
    return true;
}

static bool GetChannels(ani_env *env, ani_object aniObj, std::set<OHOS::AudioStandard::AudioChannel> &out)
{
    std::vector<int32_t> valArray;
    ...
    if (valArray.size() > 0) {
        out = {static_cast<OHOS::AudioStandard::AudioChannel>(valArray[0])};
    }
    return true;
}

static bool GetChannelMasks(ani_env *env, ani_object aniObj, int32_t &out)
{
    std::vector<int32_t> valArray;
...
int32_t TaiheUtils::GetAudioDeviceDescriptor(uintptr_t in, OHOS::AudioStandard::AudioDeviceDescriptor &out)
{
    ...
    CHECK_RETURN(GetSampleRate(env, aniObj, out.audioStreamInfo_.samplingRate) == OHOS::AVSession::AVSESSION_SUCCESS,
        "GetAudioDeviceDescriptor get sampleRates failed", OHOS::AVSession::ERR_INVALID_PARAM);
    CHECK_RETURN(GetChannels(env, aniObj, out.audioStreamInfo_.channels) == OHOS::AVSession::AVSESSION_SUCCESS,
