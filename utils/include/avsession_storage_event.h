...
// recordCount/recordSizeKB come from in-memory bookkeeping (writes recorded since the
// last report), so the receiver can distinguish "new writes this period" from "total on disk".
struct StorageStatistics {
    size_t totalCount = 0;
    int64_t totalSizeKB = 0;
    size_t localCount = 0;
    int64_t localSizeKB = 0;
    size_t castCount = 0;
    int64_t castSizeKB = 0;
    size_t historyCount = 0;
    int64_t historySizeKB = 0;
    size_t recordCount = 0;
    int64_t recordSizeKB = 0;
};

class StorageUserData {
public:
    void AddFileInfo(const std::string& fileName, int64_t fileSizeKB, const std::string& bundleName);
    void RemoveFileInfo(const std::string& fileName);
    void ClearFileInfo();
    size_t GetTotalFileCount();
    // Sum of fileSizeKB_ across all recorded files (in-memory bookkeeping, not disk scan).
    int64_t GetTotalStorageKB();
    // Appends one serialized string per recorded file to the given vector. Each string is
    // formatted as "FILE|NAME=..|SIZE=..|BUNDLE=.." for the receiver to parse.
...
public:
    ...
    void RecordFileWrite(const std::string& fileName, int64_t fileSizeBytes,
        const std::string& bundleName, int32_t userId);
    void RecordFileDelete(const std::string& fileName, int32_t userId);
    void ReportStorageStatistics();
    void ReportStorageStatisticsForUser(int32_t userId);
...
private:
    ...
    int64_t BytesToKB(int64_t bytes);
...
#define STORAGE_EVENT_RECORD_FILE_WRITE(fileName, fileSizeBytes, bundleName, userId)               \
    do {                                                                                           \
        OHOS::AVSession::AVSessionStorageEvent::GetInstance().RecordFileWrite(                     \
            fileName, fileSizeBytes, bundleName, userId);                                          \
    } while (0)
...
#else
#define STORAGE_EVENT_RECORD_FILE_WRITE(fileName, fileSizeBytes, bundleName, userId) \
    do { (void)(fileName); (void)(fileSizeBytes); (void)(bundleName); (void)(userId); } while (0)
#define STORAGE_EVENT_RECORD_FILE_DELETE(fileName, userId) \
