...
#ifndef OHOS_ABILITY_RUNTIME_JS_MEDIA_CONTROL_EXTENSION_H
#define OHOS_ABILITY_RUNTIME_JS_MEDIA_CONTROL_EXTENSION_H
...
class MediaControlExtension;
class JsRuntime;
/**
 * @brief Basic ui extension components.
 */
class JsMediaControlExtension : public MediaControlExtension,
  public std::enable_shared_from_this<JsMediaControlExtension> {
public:
    explicit JsMediaControlExtension(JsRuntime& jsRuntime);
    virtual ~JsMediaControlExtension() override;

    /**
     * @brief Create JsMediaControlExtension.
     *
     * @param runtime The runtime.
     * @return The JsMediaControlExtension instance.
     */
    static JsMediaControlExtension* Create(const std::unique_ptr<Runtime>& runtime);
...
private:
    ...
    NativeValue* CallObjectMethod(const char* name, NativeValue * const *argv = nullptr,
      size_t argc = 0);
    ...
    std::unordered_map<uint32_t, sptr<Rosen::Window>> uiWindowMap_;
    std::unordered_set<uint64_t> foregroundWindows_;
};
}  // namespace AbilityRuntime
}  // namespace OHOS
#endif  // OHOS_ABILITY_RUNTIME_JS_MEDIA_CONTROL_EXTENSION_H
