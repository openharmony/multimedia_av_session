...
void AVSessionDumper::ShowMetaData(std::string& result, const AVSessionService& sessionService)
{
    ...
    for (const auto& it : sessionService.controllers_) {
        ...
        for (const auto& item : it.second) {
            ...
            result.append("        fast forward skip intervals : " +
                std::to_string(metaData.GetFastForwardSkipIntervals()) + "\n");
            result.append("        rewind skip intervals : " +
                std::to_string(metaData.GetRewindSkipIntervals()) + "\n");
