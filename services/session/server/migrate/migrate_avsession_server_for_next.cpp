...
{
    CHECK_AND_RETURN_RET_LOG(isNeedByRemote.load(), false, "noNeed");
    std::lock_guard lockGuard(cacheJsonLock_);
...
void MigrateAVSessionServer::HandleLongPauseDetection(const AVPlaybackState &state)
{
    if (!state.GetMask().test(AVPlaybackState::PLAYBACK_KEY_STATE)) {
        return;
    }
    ...
    if (newState == lastPlaybackState_) {
        return;
    }
    lastPlaybackState_ = newState;
    if (newState == AVPlaybackState::PLAYBACK_STATE_PAUSE) {
        auto timerCallback = [weakSelf = std::weak_ptr<MigrateAVSessionServer>(shared_from_this())]() {
            auto self = weakSelf.lock();
            if (self) {
                self->HandleLongPauseTimer();
            }
        };
        AVSessionEventHandler::GetInstance().AVSessionReplaceTask(timerCallback, "LongPauseTimer",
            longPauseTimerInterval);
        SLOGI("start 5min long pause timer");
    } else if (newState == AVPlaybackState::PLAYBACK_STATE_PLAY) {
        AVSessionEventHandler::GetInstance().AVSessionRemoveTask("LongPauseTimer");
        SLOGI("reset long pause timer");
        if (hasLongPauseNotified_.load()) {
            SendLongPauseNotifyToNext(false);
            SLOGI("send long pause resume to watch");
...
{
    HandleLongPauseDetection(state);
    CHECK_AND_RETURN_LOG(CheckPostPlaybackState(state), "stateNoChange");
...
bool MigrateAVSessionServer::CheckPostSessionInfo(const std::string& sessionId)
{
    ...
    CHECK_AND_RETURN_RET_LOG(preSessionId_ != sessionId, false, "session:%{public}s no change",
        SoftbusSessionUtils::AnonymizeDeviceId(preSessionId_).c_str());
    preSessionId_ = sessionId;
    return true;
}
...
bool MigrateAVSessionServer::CheckSyncSessionInfo(const std::string& sessionId)
{
    ...
    if (!isNeedByRemote.load()) {
        if (!sessionIdCache_.empty() || sessionId.empty()) {
            return false;
        }
    }
...
void MigrateAVSessionServer::PostSyncFocusSessionInfo(std::string msg)
{
    MigratePostTask(
        [weakSelf = std::weak_ptr<MigrateAVSessionServer>(shared_from_this()), msg = std::move(msg)]() {
            auto self = weakSelf.lock();
            if (self) {
...
void MigrateAVSessionServer::SendOrCacheSessionInfo(bool needSync, std::string msg)
{
    if (needSync) {
        {
            std::lock_guard lockGuard(cacheJsonLock_);
        ...
        PostSyncFocusSessionInfo(std::move(msg));
    } else {
        std::lock_guard lockGuard(cacheJsonLock_);
        pendingSessionInfo_ = std::move(msg);
...
void MigrateAVSessionServer::UpdateSessionInfoToRemote(sptr<AVControllerItem> controller)
{
    ...
    bool needSync = CheckSyncSessionInfo(controller->GetSessionId());
    ...
    cJSON_Delete(sessionInfo);
    SendOrCacheSessionInfo(needSync, std::move(msg));
    if (needSync) {
...
void MigrateAVSessionServer::UpdateEmptyInfoToRemote()
{
    ...
    bool needSync = CheckSyncSessionInfo("");
    cJSON* sessionInfo = SoftbusSessionUtils::GetNewCJSONObject();
    CHECK_AND_RETURN_LOG(sessionInfo != nullptr, "get sessionInfo json with nullptr");
...
void MigrateAVSessionServer::ProcessMediaControlTimerRequest(cJSON* commandJsonValue)
{
    ...
    if (isNeedByRemote.compare_exchange_strong(expected, true)) {
        ...
        bool hasPending = false;
        {
            ...
            if (!pendingSessionInfo_.empty()) {
                msg = std::move(pendingSessionInfo_);
                pendingSessionInfo_.clear();
    ...
    AVSessionEventHandler::GetInstance().AVSessionReplaceTask(timerCallback, "NeedStateTimer", timeoutMs);
