    ...
    const std::shared_ptr<AAFwk::WantParams>& extras)
{
    ...
    auto didlStr = getExtraStr(ExtraKeyEvent::DIDL_LITE);
    auto metadataStr = getExtraStr(ExtraKeyEvent::CURRENT_URI_METADATA);
    ...
    cJSON_AddStringToObject(extrasObj, ExtraKeyEvent::DIDL_LITE, didlStr.c_str());
    cJSON_AddStringToObject(extrasObj, ExtraKeyEvent::CURRENT_URI_METADATA, metadataStr.c_str());
...
int32_t HwCastStreamPlayer::GetRecommendedResolutionLevel(std::string& decoderType, ResolutionLevel& resolutionLevel)
{
    ...
    if (it != jsonCapabilitiesSptr_->decoderSupportResolutions_.end()) {
        ...
        CHECK_AND_RETURN_RET_LOG(!resolutionLevels.empty(), AVSESSION_ERROR,
            "resolutionLevels is empty for decoderType: %{public}s", decoderType.c_str());
