...
void PcmCastSession::OnSystemCommonEvent(const std::string& args) {
    ...
    CHECK_AND_RETURN_RET_LOG(castState_ != 0, void(), "First connection, cast State is 0.");
    ...
    SLOGI("Reviced HIPLAY_CONFIG_MODE_DATA: castMode=%{puclic}d, uid=%{puclic}d, deviceId:%{public}s",
    ...
    std::pair<std::string, std::string>& serviceNameStatePair, const SessionToken& sessionToken)
{
    ...
    if (castHandle_ > 0) {
        if (castHandleDeviceId == outputDeviceInfo.deviceInfos_[0].deviceId) {
    ...
    if (ret == AVSESSION_SUCCESS) {
        ...
        SLOGI("PcmCastSession StartCast castHandleDeviceId_:%{public}",
...
int32_t PcmCastSession::SendStateChangeRequest(const SessionToken& sessionToken)
{
    ...
    SLOGI("SendStateChangeRequest, sessionId=%{public}s, uid=%{piblic}d, castMode=%{public}d",
...
void PcmCastSession::ExecuteCommonCommand(const std::string& commonCommand, const AAFwk::WantParams& commandArgs)
{
    ...
    CHECK_AND_CONTINUE_LOG(it != COMMON_COMMAND_MAPS.end(), "commonCommand is not support");
